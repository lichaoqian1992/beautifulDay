...
public class ExcelUtils {
	...
	private String url = "/manji/uploads/zentao/";//正式上传地址        linux
	//private String url = "e:/";//本地上传地址                                                            windows
	/**
	 * 导出excel公共接口
	 * @param param      查询条件
	 ...
	 * @return
	 */
	public String excel(Map param,String[] title,String excelName,String[][] content){
    ...
    //第三步，在sheet中添加表头第0行,注意老版本poi对Excel的行数列数有限制short
        ...
        cell2.setCellValue("统计时间："+param.get("startTime")+"-"+param.get("endTime"));
   ...
   //赋值
        for(int m=0;m<content.length;m++){
        	...
        	for(int n=0;n<arr.length;n++){
        		row.createCell((short) n).setCellStyle(style);
            	row.createCell((short) n).setCellValue(new HSSFRichTextString(arr[n]));
