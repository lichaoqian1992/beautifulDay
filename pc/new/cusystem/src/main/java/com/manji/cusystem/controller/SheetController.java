...
public class SheetController extends BaseController{
    ...
    public String findSheets(@Valid QuerySheetsVo vo,BindingResult bindingResult,HttpServletRequest request){
        ...
        if(result.getCode().equals("200")){
            if(bindingResult.hasErrors()){
                ...
                Account user = (Account)result.getResult();
                result = service.findSheets(vo,user);
            }
    ...
    public String selectSheetDetail(@Param("sessionId")String sessionId,@Param("sheetId")String sheetId){

        BaseResult result = logins(sessionId);
        if(result.getCode().equals("200")){
            Account user = (Account)result.getResult();
            result = service.selectSheetDetail(user,sheetId);
    ...
    public String remark(@Param("sessionId")String sessionId,@Param("sheetId")String sheetId,@Param("remark")String remark){

        BaseResult result = logins(sessionId);
        if(result.getCode().toString().equals("200")){
            Account user = (Account)result.getResult();
            result = service.remark(sheetId,user,remark);
        }
        return JSONObject.toJSONString(result,SerializerFeature.WriteMapNullValue);
    }
