...
public class UserService implements IUserService {
            ...
            if (usernameisok){
                ...
                if (passwordisok){
                    ...
                    if (pjtcodeisok){//全部校验通过(登录成功)
                        ...
                        User user=findUserByusernameAndpassword(username,password,code);//开始处理信息
                        u= JSONObject.toJSONString(user);//User信息对应的json
     ...
     * @return
     */
     public String findMenuByUserId(int user_id,int type_id,String pjt_code){

         List<MenuJson> menuJsonList=menuMapper.findMenuByUserId(user_id,type_id,pjt_code);
         String json = JSON.toJSONString(menuJsonList);
         return json;
     }
     ...
     */
    @Override
    public String findRolenameAndDeptname(int user_id) {
        List<RoleDepartment> roleDepartmentList=userMapper.findRolenameAndDeptname(user_id);
        String json = JSON.toJSONString(roleDepartmentList);
        return json;
    }
     ...
     */
    @Override
    public String findGroupByUserId(int userId) {
        List<Group> groupList=groupMapper.findGroupByUserId(userId);
        String json = JSON.toJSONString(groupList);
        return json;
    }
    ...
    public boolean delectUsers(int userId) {
        boolean isok=false;
        try {
            userMapper.delectUserRole(userId);
            isok=userMapper.delectUsersById(userId);
        }catch (Exception e){
            e.printStackTrace();
            return false;
        }
