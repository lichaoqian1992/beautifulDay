...
public class AckServiceImpl implements AckService{
    ...
    public BaseResult addTree(int pid, int menuId, String title) {
        ...
        boolean isTree;
        try{
            int count=ackServiceFeignService.findInformationByTreeCount(pid);//查询父节点上的信息数量
            if (count>0){
                baseResult.setCode("500");
                baseResult.setContent("添加节点失败,此节点下有信息不能添加子节点");
                return baseResult;
            }
    ...
    public BaseResult insertInformation(String title, String content, String category, int treeId, int state) {
        boolean isInfo;
        try{
            if (ackServiceFeignService.isTree(treeId,pjtCode)){//如果此节点下有其他节点则不能添加信息
                baseResult.setCode("500");
                baseResult.setContent("添加信息失败,此节点下有其他节点不能添加信息!");
                return baseResult;
            }
    ...
    public BaseResult updateInformationState(int state, int id) {
        ...
        try {
            ...
            e.printStackTrace();
            baseResult.setCode("500");
            baseResult.setContent("服务器错误,修改信息状态失败");
        }
        return baseResult;
    }
     ...
     */
    @Override
    public BaseResult findAllTree() {
        try{
            JSONArray jsonArray=ackServiceFeignService.findAllTree(pjtCode);
            baseResult.setCode("200");
            baseResult.setContent("查询失败");
            baseResult.setResult(jsonArray);
        }catch (Exception e){
            baseResult.setCode("500");
            baseResult.setContent("查询失败");
